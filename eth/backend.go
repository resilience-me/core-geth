...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	engine := ethconfig.CreateConsensusEngine(stack, &ethashConfig, panarchyConfig, cliqueConfig, lyra2Config, config.Miner.Notify, config.Miner.Noverify, chainDb)
...
func (s *Ethereum) StartMining(threads int) error {
	...
	if !s.IsMining() {
		...
		var pan *panarchy.Panarchy
		var cli *clique.Clique

		switch e := s.engine.(type) {
		case *panarchy.Panarchy:
		    pan = e
		case *clique.Clique:
		    cli = e
		case *beacon.Beacon:
		    switch inner := e.InnerEngine().(type) {
		    case *panarchy.Panarchy:
			pan = inner
		    case *clique.Clique:
			cli = inner
		...
		if pan != nil || cli != nil {
		    ...
		    if pan != nil {
			pan.Authorize(eb, wallet.SignData)
		    } else if cli != nil {
